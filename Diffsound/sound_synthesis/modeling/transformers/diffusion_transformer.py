...
def reshape_heads_to_batch_dim(heads, tensor):
...
def reshape_batch_dim_to_heads(heads, tensor):
    batch_size, seq_len, dim = tensor.shape
    head_size = heads
...
def register_attention_control(self, controller):
    def ca_forward(self):
        def forward(x, encoder_output,  mask=None):
            B, T, C = x.size()
            B, T_E, _ = encoder_output.size()
            is_cross = encoder_output is not None
            encoder_output = encoder_output if is_cross else x
            ...
            k = reshape_heads_to_batch_dim(self.n_head, k)
            q = reshape_heads_to_batch_dim(self.n_head, q)
            v = reshape_heads_to_batch_dim(self.n_head, v)

            # att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))  # (B, nh, T, T)
            att = torch.einsum("b i d, b j d -> b i j", q, k) * (1.0 / math.sqrt(k.size(-1)))
            att = F.softmax(att, dim=-1)  # (B, nh, T, T)

            if mask is not None:
                mask = mask.reshape(B, -1)
                max_neg_value = -torch.finfo(att.dtype).max
                mask = mask[:, None, :].repeat(self.n_head, 1, 1)
                att.masked_fill_(~mask, max_neg_value)
            ...
            # ここでコントローラーforward
            att = controller(att, is_cross)
            att = self.attn_drop(att)
            y = att @ v  # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
            # y = y.transpose(1, 2).contiguous().view(B, T, C)  # re-assemble all head outputs side by side, (B, T, C)

            y = reshape_batch_dim_to_heads(self.n_head, y)
...
class DiffusionTransformer(nn.Module):
    ...
    def sample(
        ...
        if content_token == None:
            ...
            start_step = self.num_timesteps
            tmp = input['content_token'][0]
            input['content_token'] = tmp.repeat(batch_size,1)
            sample_image = input['content_token'].type_as(input['content_token'])  # B,265
